    ...
    () => [
      ...
      },
      {
        Cell: ({
          ...
          const editors = Array.isArray(original.editors)
            ? original.editors
            : [];
          const owners = Array.isArray(original.owners) ? original.owners : [];

          return <FacePile users={editors.length > 0 ? editors : owners} />;
        },
        Header: t('Owners'),
        id: 'editors',
        disableSortBy: true,
      ...
      },
      {
        Cell: ({ row: { original } }: CellProps<Dataset>) => {
          ...
          const ownedUsers = Array.isArray(original.editors)
            ? original.editors
            : Array.isArray(original.owners)
              ? original.owners
              : [];
          const allowEdit =
            ownedUsers.map((o: Owner) => o.id).includes(Number(user.userId)) ||
            isUserAdmin(user);
