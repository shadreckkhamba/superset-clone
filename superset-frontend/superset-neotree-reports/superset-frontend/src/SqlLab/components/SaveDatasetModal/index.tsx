    ...
    try {
      const [, key] = await Promise.all([
        updateDataset({
          ...
          editors: (datasetToOverwrite?.editors ??
            datasetToOverwrite?.owners ??
            []
          ).map((o: DatasetOwner) => o.id),
      ...
      return SupersetClient.get({
        ...
        data: response.json.result.map(
          (r: {
            ...
            id: number;
            owners?: [DatasetOwner];
            editors?: [DatasetOwner];
          }) => ({
            value: r.table_name,
            label: r.table_name,
            datasetid: r.id,
            owners: r.owners ?? r.editors ?? [],
            editors: r.editors ?? r.owners ?? [],
          }),
        ),
