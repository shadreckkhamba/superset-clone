    ...
    if (compareIndex < sortedData.length) {
      ...
      if (compareToValue !== null && compareFromValue !== null) {
        percentChange = compareFromValue
          ...
          : 0;
        const trendArrow = percentChange >= 0 ? '↑' : '↓';
        const absPercent = formatPercentChange(Math.abs(percentChange));
        const prevFormatted = numberFormatter(compareFromValue);
        const suffix = compareSuffix ? ` ${compareSuffix}: ` : ': ';
        formattedSubheader = `${trendArrow} ${absPercent}${suffix}${prevFormatted}`;
      }
    }
  }
