...
const PropertiesModal = ({
  ...
  const loadAccessOptions = useCallback(
    (accessType = 'editors', input = '', page: number, pageSize: number) => {
  ...
  const handleDashboardData = useCallback(
    dashboardData => {
      ...
      const accessOwners = editors ?? owners;
      const dashboardInfo = {
        id,
          ...
          <StyledFormItem label={t('Owners')}>
            <AsyncSelect
              ...
              options={(input, page, pageSize) =>
                loadAccessOptions('editors', input, page, pageSize)
              }
