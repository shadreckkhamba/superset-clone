...
test('Should not crash when a cell renderer throws', () => {
  const columns = [
    {
      Header: 'Broken',
      accessor: 'broken',
  ...
  const data = [{ broken: 'value' }];
  // @ts-ignore
  const tableHookResult = renderHook(() => useTable({ columns, data }));
  const props = {
    prepareRow: tableHookResult.result.current.prepareRow,
    headerGroups: tableHookResult.result.current.headerGroups,
    rows: tableHookResult.result.current.rows,
    columns: tableHookResult.result.current.columns,
    loading: false,
    getTableProps: jest.fn(),
    getTableBodyProps: jest.fn(),
  };
  ...
  expect(() => render(<TableCollection {...props} />)).not.toThrow();
