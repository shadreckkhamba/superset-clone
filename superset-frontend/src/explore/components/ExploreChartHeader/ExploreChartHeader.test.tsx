...
describe('ExploreChartHeader', () => {
  ...
  test('renders metadata when owners are missing', async () => {
    const props = createProps({ showTitlePanelItems: true });
    render(
      <ExploreHeader
        {...props}
    ...
    const addedToOneDashboard = await screen.findByText(
      'Added to 1 dashboard',
    );
    expect(addedToOneDashboard).toBeInTheDocument();
    userEvent.hover(screen.getByText('John Doe'));
