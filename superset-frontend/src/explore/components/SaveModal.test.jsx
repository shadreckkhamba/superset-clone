...
test('loads dashboards using the editors filter', async () => {
  const getSpy = jest.spyOn(SupersetClient, 'get').mockResolvedValue({
    json: {
      result: mockDashboardData.result,
      count: 1,
    },
  });

  try {
    const saveModal = new PureSaveModal({
      ...defaultProps,
      user: { userId: 1 },
      dispatch: jest.fn(),
      dashboardId: null,
    ...
    const response = await saveModal.loadDashboards('sales', 0, 100);
    ...
    expect(response).toEqual({
      data: [{ label: 'dashboard title', value: 'id' }],
      totalCount: 1,
    });
  } finally {
    getSpy.mockRestore();
