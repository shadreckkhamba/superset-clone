...
test('disables overwrite option when there is no existing slice', () => {
  const { getByRole } = setup(
    {},
    mockStore({
      ...initialState,
...
test('shows the backend error when saving the chart fails', async () => {
  const dashboardId = 123;
  const createSlice = jest.fn().mockRejectedValue(
    new Response(JSON.stringify({ error: 'Chart creation failed' }), {
      status: 400,
      headers: { 'Content-Type': 'application/json' },
    }),
  );

  const dashboardEndpoint = `glob:*/api/v1/dashboard/${dashboardId}`;
  const tabsEndpoint = `glob:*/api/v1/dashboard/${dashboardId}/tabs`;
  ...
  fireEvent.click(getByRole('radio', { name: 'Save as...' }));
  await waitFor(() =>
    expect(fetchMock.callHistory.calls(dashboardEndpoint)).toHaveLength(1),
  );
  fireEvent.click(getByRole('button', { name: 'Save' }));
