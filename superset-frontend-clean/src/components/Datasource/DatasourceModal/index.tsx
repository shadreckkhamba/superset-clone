...
const DatasourceModal: FunctionComponent<DatasourceModalProps> = ({
  ...
  const buildPayload = (datasource: Record<string, any>) => {
    const payload: Record<string, any> = {
      ...
      editors: ((datasource.editors ?? datasource.owners) || []).map(
  ...
  const onConfirmSave = async () => {
    ...
    try {
      ...
      const editors = currentDatasource.editors ?? currentDatasource.owners ?? [];
      onDatasourceSave({
        ...json.result,
        editors: json.result.editors ?? editors,
        owners: json.result.owners ?? editors,
      });
      onHide();
    } catch (response) {
      setIsSaving(false);
